...
(function (window, document, undefined) {
    ...
    L.LatLngGraticule = L.Layer.extend({
            ...
            if (L.Browser.canvas && map) {
                ...
                function __draw_lat_line(self, lat_tick) {
                    ...
                    if (curvedLat) {
                        ...
                        if (self.options.showLabel && label) {
                            var latstrDMS = self._convertDMSLat(lat_tick);
                            var _yy = ll.y + (txtHeight)-2;
                            //var _yy = ll.y + (txtHeight/2)-2;
                            ctx.fillText(latstrDMS, 0, _yy);
                            ctx.fillText(latstrDMS, ww-txtWidth, _yy);
                        }
                    }
                };
                ...
                function __draw_lon_line(self, lon_tick) {
                    ...
                    if (curvedLon) {
                        ...
                        if (self.options.showLabel && label) {
                            var lngstrDMS = self._convertDMSLng(lon_tick);
                            ctx.fillText(lngstrDMS, tt.x - (txtWidth/2), txtHeight+1);
                            ctx.fillText(lngstrDMS, bb.x - (txtWidth/2), hh-3);
                        }
                    }
                };
        ...
        _convertDMSLat: function (lat) {
            var latitude = this._toDegreesMinutesAndSeconds(lat);
            var latitudeCardinal = Math.sign(lat) >= 0 ? "N" : "S";

            return latitude + " " + latitudeCardinal;
        },
        _convertDMSLng: function (lng) {
            var longitude = this._toDegreesMinutesAndSeconds(lng);
            var longitudeCardinal = Math.sign(lng) >= 0 ? "E" : "W";

            return longitude + " " + longitudeCardinal;
        },
        _toDegreesMinutesAndSeconds: function (coordinate) {
